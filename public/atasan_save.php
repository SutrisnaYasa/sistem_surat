...
$kodeSimpanUser = $_REQUEST['kodeSimpanUser'];


if($kodeSimpanUser=='text'){
	...
	$save = mysqli_query($config, "INSERT INTO sip_atasan(noSurat, kodeBidang, pengirim, tglInput, userPengirim, status, tglStatus, fileDisposisi, tglInputFile,  userInputFile) VALUES ('$noSurat', '$kodeBidang', '$kodeBidangPe', NOW(), '$userPengirim', '0', NOW(), '', NOW(), '')");

	if($save == true){				
					echo "<script>
					alert('Disposisi Berhasil.');location.href='inbox_detail.php?idSurat=$idSurat&&kategoriSurat=$kategoriSurat';
					</script>";
				}else{
					echo "<script>
